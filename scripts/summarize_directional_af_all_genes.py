#!/usr/bin/env python3
"""Summarize directional epistasis scores against pair-level allele-frequency metrics.

The script combines per-gene epistasis outputs, derives haplotype-based allele
frequency summaries, ranks variant pairs by epistatic score, and generates
summary tables and figures across deleteriousness bins.
"""
...
def merge_one_gene(base_dir, gene):
    ...
    epi_path = find_file_case_insensitive(
        gene_dir, gene, ["_epistasis_esmcnd.tsv", "_epistasis_esmcnd.tsv.gz"]
    )
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        help="Base directory containing per-gene folders"
    )
    parser.add_argument(
        "--out-dir",
        default=None,
        help="Output directory; default: BASE_DIR/directional_af_summary_all_genes_CI"
    )
    parser.add_argument(
        "--score-col",
        default="E_cond",
    ...
    out_dir = args.out_dir or os.path.join(
        base_dir,
        "directional_af_summary_all_genes_CI"
    )
    ...
    combined_path = os.path.join(
        out_dir,
        "combined_directional_with_pair_af_all_genes.tsv.gz"
    )
    ...
    summary_path = os.path.join(
        out_dir,
        "directional_af_bin_summaries_all_genes.tsv"
    )
    ...
    df[df["score_rank_pct"] > 0.10].to_csv(
        os.path.join(out_dir, "rest_pairs_directional.tsv.gz"),
        sep="\t",
        index=False,
        compression="gzip"
    )
    ...
    corr_path = os.path.join(
        out_dir,
        "spearman_score_vs_pair_af_all_genes.tsv"
    )
    ...
    make_scatter(
        ...
        args.score_col,
        "pair_mean_af",
        os.path.join(out_dir, f"scatter_pair_mean_af_vs_{args.score_col}.png")
    )
    ...
    make_scatter(
        ...
        args.score_col,
        "pair_mean_af",
        os.path.join(out_dir, f"scatter_pair_mean_af_vs_{args.score_col}_raw_AF_log_y_axis.png"),
        yscale="log"
    )
    ...
    make_barplot(
        summary_df,
        "pair_mean_af",
        os.path.join(out_dir, "MAIN_bar_pair_mean_af_by_score_bin_median_raw_AF_log_y_axis.png"),
        stat="median",
    ...
    make_barplot(
        summary_df,
        "pair_mean_af",
        os.path.join(out_dir, "MAIN_bar_pair_mean_af_by_score_bin_mean_raw_AF_log_y_axis.png"),
        stat="mean",
    ...
    make_barplot(
        summary_df,
        "pair_mean_af",
        os.path.join(out_dir, "bar_pair_mean_af_by_score_bin_median_raw_AF_linear_y_axis.png"),
        stat="median",
        title="Median raw mean pair AF across epistatic score bins",
        ylabel="Median raw mean pair AF"
    )
    ...
    make_barplot(
        summary_df,
        "pair_mean_af",
        os.path.join(out_dir, "bar_pair_mean_af_by_score_bin_mean_raw_AF_linear_y_axis.png"),
        stat="mean",
        title="Mean raw mean pair AF across epistatic score bins",
        ylabel="Mean raw mean pair AF"
    )
    ...
    make_scatter(
        ...
        args.score_col,
        "log10_pair_mean_af",
        os.path.join(out_dir, f"QC_scatter_log10_pair_mean_af_vs_{args.score_col}.png")
    )

    make_barplot(
        summary_df,
        "log10_pair_mean_af",
        os.path.join(out_dir, "QC_bar_log10_pair_mean_af_by_score_bin_median.png"),
        stat="median",
    ...
    make_barplot(
        summary_df,
        "depletion_score",
        os.path.join(out_dir, "bar_depletion_score_by_score_bin_median.png"),
        stat="median",
    ...
    run_summary = pd.DataFrame([{
        ...
        "recommended_plot": "MAIN_bar_pair_mean_af_by_score_bin_median_raw_AF_log_y_axis.png",
    ...
    run_summary.to_csv(
        os.path.join(out_dir, "run_summary_all_genes.tsv"),
        sep="\t",
        index=False
    )
    ...
    print("Most useful plot: MAIN_bar_pair_mean_af_by_score_bin_median_raw_AF_log_y_axis.png", flush=True)
