#!/usr/bin/env python3
"""
Run conditional epistasis / non-additivity scoring using VESM-35M.
...
Output per gene:
  {BASE_DIR}/{GENE}/{GENE}_epistasis_VESM_35M.tsv
  {BASE_DIR}/{GENE}/{GENE}_epistasis_skipped_rows_VESM_35M.tsv

This script does NOT overwrite the original:
...
VESM_CHECKPOINT = os.environ.get("VESM_CHECKPOINT", "")
...
START_BATCH_SIZE = int(os.environ.get("BATCH_SIZE", "4096"))
MIN_BATCH_SIZE = int(os.environ.get("MIN_BATCH_SIZE", "256"))
...
def verify_mut_against_seq(seq: str, mut: str, seq_name: str = "sequence") -> None:
    ...
    if seq_wt != wt:
        raise ValueError(
            f"{mut}: WT amino acid mismatch in {seq_name}. "
            f"Expected {wt} at position {pos}, found {seq_wt}"
        )
...
def build_masked_sequence(
    ...
    if context_seq[i] != wt:
        raise ValueError(
            f"{target_mut}: WT amino acid mismatch in context. "
            f"Expected {wt} at position {pos}, found {context_seq[i]}"
        )
        ...
        if os.path.exists(gene_list_default):
            ...
            genes = sorted(
                ...
                and d not in {
                    "logs",
                    "chunk_summaries",
                    "3d_linear_contact_all_genes_FINAL",
                    "gnomad_cis_epistasis_all_genes_Michel_results",
                    "alphafold_pdbs"
                }
            )
...
def load_model_and_tokenizer():
    print("[INFO] Loading VESM_35M only", flush=True)
    print(f"[INFO] Base architecture: {BASE_CHECKPOINT}", flush=True)
    print(f"[INFO] VESM checkpoint: {VESM_CHECKPOINT}", flush=True)

    if not VESM_CHECKPOINT:
        raise ValueError("Set VESM_CHECKPOINT to the path of the VESM-35M checkpoint file.")

    if not os.path.exists(VESM_CHECKPOINT):
        raise FileNotFoundError(f"VESM checkpoint not found: {VESM_CHECKPOINT}")
    ...
    print("[INFO] Loaded VESM_35M checkpoint.", flush=True)
    print(f"[INFO] Missing keys: {len(missing)}", flush=True)
    print(f"[INFO] Unexpected keys: {len(unexpected)}", flush=True)
    ...
    for batch_idx, batch in enumerate(chunked(requests, batch_size), start=1):
        ...
        for i, req in enumerate(batch):
            ...
            if len(mask_positions) != 1:
                raise ValueError(
                    f"Expected exactly 1 mask token for request {req['key']}, "
        ...
        if batch_idx == 1 or batch_idx % 10 == 0 or batch_idx == total_batches:
            print(
                f"[INFO] Finished model batch {batch_idx}/{total_batches} "
...
def score_requests_with_fallback(
    ...
    while batch_size >= min_batch_size:
        ...
        try:
            ...
            if is_cuda_oom(e):
                print(
                    f"[WARN] CUDA OOM at batch_size={batch_size}; retrying with half",
                    flush=True
                )
    ...
    raise RuntimeError(
        f"All batch sizes failed with CUDA OOM down to MIN_BATCH_SIZE={min_batch_size}"
    )
...
def process_gene(gene: str, tokenizer, model) -> Dict:
    ...
    out_path = os.path.join(
        gene_dir,
        f"{gene}_epistasis_VESM_35M.tsv"
    )

    skipped_path = os.path.join(
        gene_dir,
        f"{gene}_epistasis_skipped_rows_VESM_35M.tsv"
    )
    ...
    if n_input_rows == 0:
        return {
            "gene": gene,
            "status": "SKIPPED_ZERO_PAIRS",
            "error": ""
        }

    valid_df, skipped_df = filter_valid_rows(
        df,
        wt_seq,
        gene
    )
    ...
    request_keys = sorted(
        request_keys,
        key=lambda x: ("" if x[0] is None else x[0], x[1])
    )
    ...
    for bg_mut, target_mut in request_keys:
        ...
        context_seq = build_context_seq(
            wt_seq,
            bg_mut
        )

        masked_seq, wt_aa, mut_aa, pos = build_masked_sequence(
            context_seq,
            target_mut,
            mask_token
        )
...
def main():
    ...
    print("MODEL: VESM_35M", flush=True)
    print(f"BASE_CHECKPOINT: {BASE_CHECKPOINT}", flush=True)
    print(f"VESM_CHECKPOINT: {VESM_CHECKPOINT}", flush=True)
    ...
    print(
        f"GENE_LIST: {GENE_LIST_PATH if GENE_LIST_PATH else '[BASE_DIR/all_genes_gene_list.txt or all subdirs]'}",
        flush=True
    )
    ...
    os.makedirs(
        os.path.join(BASE_DIR, "chunk_summaries"),
        exist_ok=True
    )
    ...
    summary_path = os.path.join(
        BASE_DIR,
        "chunk_summaries",
        f"chunk_{TASK_ID}_epistasis_summary_VESM_35M.tsv"
    )

    failures_path = os.path.join(
        BASE_DIR,
        "chunk_summaries",
        f"chunk_{TASK_ID}_epistasis_failures_VESM_35M.tsv"
    )
